...
def floodFill(x, y, xprev, yprev):
    ...
    log("start of iteration" + str(stack))

    while (len(stack) != 0):
        log("after iteration" + str(stack))

        yrun = stack.pop()
        xrun = stack.pop()
        log(str(stack))
        log(str(yrun) + ", " + str(xrun))

        if isConsistant(xrun, yrun):
            log("is Consistant True (pass)  " + str(isConsistant(xrun, yrun)))
            pass
        else:
            makeConsistant(xrun, yrun)
            log("is Consistant False (inside else ) " + str(isConsistant(xrun, yrun)))
...
def toMove(x, y, xprev, yprev, orient):
    ...
    minVal = minVals[0]
    minCell = minVals.index(min(minVals))
    ...
    #                 minVal= minVals[i]
    #                 minCell= i
    # log("orient "+str(orient)+ " minCell  " +str(minCell))
